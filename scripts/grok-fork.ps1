...
$Bin = if ($env:GROK_FORK_BIN) { $env:GROK_FORK_BIN } else { Join-Path $Repo 'target\release\xai-grok-pager.exe' }
$Stamp = if ($env:GROK_FORK_STAMP) { $env:GROK_FORK_STAMP } else { Join-Path $Repo '.fork-built-at' }
$LockDir = if ($env:GROK_FORK_LOCK) { $env:GROK_FORK_LOCK } else { Join-Path $Repo '.fork-launch.lock' }
...
function Integrate-Upstream {
    ...
    if ($base -eq $up) {
        $upShort = (& git rev-parse --short "$UpstreamRemote/$UpstreamRef").Trim()
        $headShort = (& git rev-parse --short HEAD).Trim()
        Write-ForkLog "already up to date with $UpstreamRemote/$UpstreamRef ($upShort); local $headShort"
        return
    }
...
function Sync-Remotes {
    ...
    try {
        ...
        Write-ForkLog "fetching $OriginRemote/$OriginRef (fork) + $UpstreamRemote/$UpstreamRef ..."
        $remotes = & git remote
