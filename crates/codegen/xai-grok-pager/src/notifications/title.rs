...
impl TitleManager {
    ...
    pub fn update(&mut self, state: &TitleState<'_>) -> Option<String> {
        ...
        if !has_parts {
            self.composed.clear();
            self.composed.push_str(crate::brand::product_name());
        }
    ...
    pub fn reset(&mut self) -> String {
        let name = crate::brand::product_name();
        let esc = build_title_escape(name);
        self.last_title.clear();
        self.last_title.push_str(name);
...
fn write_item(
    ...
    match item {
        TitleItem::Grok => {
            ...
            buf.push_str(crate::brand::product_name());
        }
        TitleItem::Spinner => {
